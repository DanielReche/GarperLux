...
(function () {
  ...
  const publicAuthlessPages = new Set(['recuperar-password.html', 'contacto.html', 'trabaja-con-nosotros.html', 'seguir-solicitud.html']);
    ...
    document.querySelectorAll('button[onclick*="glxSetSession"]').forEach((button) => {
      button.addEventListener('click', async (event) => {
        ...
        try {
          ...
          location.href = new URLSearchParams(location.search).get('redirect') || 'area-personal.html';
        } catch (error) {
          toast(error.message);
        }
      }, true);
  ...
  function bindPasswordRecovery() {
    if (page !== 'recuperar-password.html') return;
  ...
  const renderProductCard = (product) => `
    <a href="producto.html?sku=${encodeURIComponent(product.sku)}" class="card-prod group" data-sku="${product.sku}">
  ...
  function bindCatalogActions() {
    document.addEventListener('click', async (event) => {
      ...
      try {
        const cart = await api.addToCart(button.dataset.sku || findSku(button), 1);
        addCartBadge(cart.items.reduce((sum, item) => sum + item.quantity, 0));
  ...
  async function bindCatalogPages() {
    if (!['tienda.html', 'categoria.html'].includes(page)) return;
    const products = await api.products(page === 'categoria.html' ? { category: 'mecanismos' } : {});
    ...
    if (grid) grid.innerHTML = products.slice(0, page === 'categoria.html' ? 12 : 8).map(renderProductCard).join('');
  }
  ...
  async function bindCartPage() {
    ...
    try {
      const cart = await api.cart();
      const mainGrid = document.querySelector('main .lg\\:grid, main .grid');
      const list = mainGrid?.querySelector('.space-y-4, .space-y-5, .space-y-6') || document.querySelector('main section');
      if (list) {
        list.innerHTML = cart.items.length ? cart.items.map((item) => `
          <article class="bg-white border border-line rounded-2xl p-4 flex gap-4" data-sku="${item.sku}">
            <div class="w-20 h-20 rounded-xl bg-paper-2 grid place-items-center font-serif text-graphite/30">GL</div>
            <div class="flex-1">
              <div class="font-mono text-[11px] text-graphite mb-1">SKU ${item.sku}</div>
              <h3 class="font-medium">${item.name}</h3>
              <div class="flex items-center justify-between mt-3">
                <span class="font-mono text-sm">x${item.quantity}</span>
                <strong class="font-serif text-xl">${money(item.price * item.quantity)}</strong>
              </div>
            </div>
          </article>`).join('') : '<div class="bg-white border border-line rounded-2xl p-8 text-center">Tu carrito está vacío.</div>';
      }
      addCartBadge(cart.items.reduce((sum, item) => sum + item.quantity, 0));
    } catch (error) {
      toast(error.message);
  ...
  async function bindProductPage() {
    ...
    try {
      ...
      const brand = document.querySelector('section a[href="marcas.html"]');
  ...
  function bindCheckout() {
    if (!['carrito.html', 'checkout.html', 'pago.html'].includes(page)) return;
    ...
    primary?.addEventListener('click', async (event) => {
      ...
      try {
        ...
        localStorage.setItem('garperlux_last_order', JSON.stringify(order));
        localStorage.removeItem(CHECKOUT_KEY);
        location.href = `pedido-confirmado.html?order=${encodeURIComponent(order.code)}`;
      } catch (error) {
        toast(error.message);
      }
    });
  ...
  function bindConfirmationPages() {
    if (page === 'pedido-confirmado.html') {
      const order = JSON.parse(localStorage.getItem('garperlux_last_order') || 'null');
      const code = new URLSearchParams(location.search).get('order') || order?.code;
      if (!code) return;
      document.querySelectorAll('*').forEach((node) => {
    ...
    if (page === 'solicitud-confirmacion.html') {
  ...
  async function bindTrackingPages() {
    ...
    if (page === 'mis-pedido.html') {
      const code = new URLSearchParams(location.search).get('order');
      if (!code || !api.getToken()) return;
      try {
        const order = await api.order(code);
        document.querySelectorAll('*').forEach((node) => {
          if (!node.children.length && /GLX-[A-Z0-9-]+|GLX-\d{2}-\d+/.test(node.textContent)) {
            node.textContent = node.textContent.replace(/GLX-[A-Z0-9-]+|GLX-\d{2}-\d+/g, order.code);
          }
        });
        const tracking = document.querySelector('.bg-white.border.border-line.rounded-2xl.p-6');
        if (tracking && order.events?.length) {
          tracking.innerHTML = `<h2 class="font-serif text-xl font-medium mb-6">Tracking real del pedido</h2><ol class="space-y-5 relative"><div class="absolute left-3 top-3 bottom-3 w-px bg-line"></div>${renderTimeline(order.events)}</ol>`;
        }
      } catch (error) {
        toast(error.message);
      }
  ...
  function bindServiceRequest() {
    if (page !== 'solicitar-tecnico.html') return;
    document.getElementById('btn-submit')?.addEventListener('click', async (event) => {
      ...
      try {
        ...
        location.href = `solicitud-confirmacion.html?code=${encodeURIComponent(request.code)}`;
      } catch (error) {
        toast(error.message);
      }
    }, true);
  ...
  function bindQuoteForms() {
    if (!['solicitar-presupuesto.html', 'crear-presupuesto.html'].includes(page)) return;
    const submitQuote = async (event) => {
      ...
      try {
        ...
        location.href = `presupuesto.html?code=${encodeURIComponent(quote.code)}`;
      } catch (error) {
        toast(error.message);
      }
    };
    ...
    if (page === 'anadir-tarjeta.html') {
      document.querySelector('form')?.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          location.href = 'metodos-pago.html';
        } catch (error) {
          toast(error.message);
        }
      }, true);
  ...
  function bindCommerceForms() {
    ...
    if (page === 'pedido-recurrente.html') {
      document.querySelector('form')?.addEventListener('submit', async (event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
        if (!requireSession()) return;
  ...
  function bindSupportForms() {
    ...
    if (page === 'trabaja-con-nosotros.html') {
      document.querySelector('form')?.addEventListener('submit', async (event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
        try {
  ...
  async function bindAccountPages() {
    if (!api.getToken()) return;
    try {
      if (page === 'mis-pedidos.html') {
        const orders = await api.orders();
        backendPanel('Pedidos reales', orders.map((order) => `<a href="mis-pedido.html?order=${order.code}" class="flex justify-between gap-4 rounded-xl bg-paper-2 p-3"><span>${order.code} · ${order.status}</span><strong>${money(order.total)}</strong></a>`), 'Todavía no hay pedidos en la base de datos.');
      }
      if (page === 'mis-solicitudes.html') {
        const requests = await api.serviceRequests();
        backendPanel('Solicitudes técnicas reales', requests.map((request) => `<a href="seguir-solicitud.html?code=${request.code}" class="flex justify-between gap-4 rounded-xl bg-paper-2 p-3"><span>${request.code} · ${request.service_type}</span><strong>${request.status}</strong></a>`), 'Todavía no hay solicitudes técnicas reales.');
      }
      if (page === 'mis-presupuestos.html') {
        const quotes = await api.quotes();
        backendPanel('Presupuestos reales', quotes.map((quote) => `<a href="presupuesto.html?code=${quote.code}" class="flex justify-between gap-4 rounded-xl bg-paper-2 p-3"><span>${quote.code} · ${quote.title}</span><strong>${money(quote.total)}</strong></a>`), 'Todavía no hay presupuestos reales.');
      }
      if (page === 'facturas.html' || page === 'albaranes.html') {
        const docs = await api.documents(page === 'facturas.html' ? 'invoice' : 'delivery_note');
        backendPanel(page === 'facturas.html' ? 'Facturas reales' : 'Albaranes reales', docs.map((doc) => `<div class="flex justify-between gap-4 rounded-xl bg-paper-2 p-3"><span>${doc.code} · ${doc.type}</span><strong>${money(doc.total)}</strong></div>`), 'Todavía no hay documentos reales.');
      }
    } catch (error) {
      toast(error.message);
    }
